...
function updateStackedBarChart() {
  const t = d3.transition().duration(1000);
  const { width, height } = config.stackedBarChart;
  const { participantId, weeks, cluster, clustered } = filter;
  let filteredData = [];
  if (clustered) {
    filteredData = groupedData.filter((d) => d.group == cluster)[0].data;
    d3.select("#stacked-bar-chart-title").text(`Cluster ${cluster}`);
  } else {
    filteredData = participantData.filter(
      (d) => d.participantId == participantId && d.weeks <= weeks
    );
    d3.select("#stacked-bar-chart-title").text(`Participant ${participantId}`);
  }
      ...
      (enter) =>
        enter
          ...
          .on("click", (event, d) => {
            ...
            filter.participantId = event.target.getAttribute(
              "data-participant-id"
            );
